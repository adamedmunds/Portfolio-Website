...
export const calculateWeakness = (type1, type2) => {
...
export const calculateStrength = (type1, type2) => {
  if (isUndefined(type1)) type1 = 'none';
  if (isUndefined(type2)) type2 = 'none';
  var result = {
    '0x': [],
  ...
  types.forEach((type, index) => {
    if (!isUndefined(type[convertType(type1)])) {
      const effectiveness = type[convertType(type1)] * type[convertType(type2)];
      result[effectiveness.toString() + 'x'].push(convertType(index));
    }
  });
  return result;
};
